...
`;
    ...
    const targetString = " * Enqueue scripts and styles.";
    ...
    const endString = "// Change Login Logo URL";
    ...
    // Include up to " * Enqueue scripts and styles." plus " */"
    let prefix = content.substring(0, targetIndex + targetString.length);
    // Find the next "*/"
    const commentEndIndex = content.indexOf("*/", targetIndex);
    if (commentEndIndex !== -1) {
        prefix = content.substring(0, commentEndIndex + 2) + "\n";
    }

    const newContent = prefix + missingCode + '\n' + content.substring(endIndex);
    
    fs.writeFileSync(filePath, newContent, 'utf-8');
    console.log("Successfully fixed functions.php");
